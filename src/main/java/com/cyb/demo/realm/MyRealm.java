...
public class MyRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals){
        ...
        SimpleAuthorizationInfo simpleAuthorInfo = new SimpleAuthorizationInfo();
        if(null!=currentUsername && "cyb".equals(currentUsername)){
            //添加一个角色，不是配置意义上的添加，而是证明该用户拥有admin角色
            simpleAuthorInfo.addRole("admin1");
            //添加权限
            simpleAuthorInfo.addStringPermission("perStr");//admin:manage 自定义组合字符串
            System.out.println("已为用户[cyb]赋予了[admin]角色和[admin:manage]权限");
            return simpleAuthorInfo;
        }
        if(null!=currentUsername && "iechenyb".equals(currentUsername)){
            System.out.println("当前用户[iechenyb]无授权（不需要为其赋予角色和权限）");
            return simpleAuthorInfo;
        }
        //若该方法什么都不做直接返回null的话
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        System.out.print("验证当前Subject时获取到token："+token);
        ...
        if("cyb".equals(token.getUsername())){
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo("cyb", "cyb", this.getName());
            this.setAuthenticationSession("cyb");
            return authcInfo;
        }
        if("iechenyb".equals(token.getUsername())){
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo("iechenyb", "iechenyb", this.getName());
            this.setAuthenticationSession("iechenyb");
            return authcInfo;
        }
